...
def get_authenticated_user(request):
    ...
    try:
        ...
        if not auth_header or not auth_header.startswith('Bearer '):
            return None, 401, "Authorization token missing or malformed"
        ...
        if user_info_header:
            try:
                ...
                # Firebase JWT typically has UID in 'user_id' claim if generated by Admin SDK,
                # or 'sub' if standard OIDC from Firebase Auth client SDK.
                # 'email' is usually present, 'email_verified' might also be useful.
        ...
        return None, 500, f"Authentication processing error: {str(e)}"
