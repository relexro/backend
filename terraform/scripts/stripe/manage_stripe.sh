...
set_env_vars() {
    ...
    # Place .env.stripe in the parent directory of SCRIPT_DIR (i.e., backend_folder/)
    ENV_FILE_PATH="${SCRIPT_DIR}/../.env.stripe"

    echo "# Stripe Price ID Environment Variables - Source this file: source .env.stripe" > "$ENV_FILE_PATH"
    echo "# Generated by manage_stripe.sh set-env on $(date)" >> "$ENV_FILE_PATH"

    print_status "Generating export commands and writing to $ENV_FILE_PATH ..."
    echo "" # Newline for console readability
    echo "# To use these variables, run: source $ENV_FILE_PATH"
    echo "" # Newline for console readability

    # Define a mapping from a reference key (used in this script) to the desired ENV_VAR_NAME
    # The reference key (e.g., "price_individual_monthly_ref") will be used to find the
    # actual lookup_key in config.json (e.g., ".prices.price_individual_monthly_ref.lookup_key")
    declare -A ref_to_env_map
    ref_to_env_map["price_individual_monthly"]="STRIPE_PRICE_ID_INDIVIDUAL_MONTHLY"
    ref_to_env_map["price_org_basic_monthly"]="STRIPE_PRICE_ID_ORGANIZATION_MONTHLY"
    # Add other mappings here if needed:
    # ref_to_env_map["price_case_tier_1_ref"]="TF_VAR_stripe_price_id_case_tier_1"

    for ref_key in "${!ref_to_env_map[@]}"; do
        env_var_name="${ref_to_env_map[$ref_key]}"
        
        # Get the actual lookup_key string from config.json using the ref_key
        # This assumes config.json has: "prices": { "price_individual_monthly": { "lookup_key": "actual_lookup_key_value", ... } }
        actual_lookup_key_val=$(jq -r ".prices.\"$ref_key\".lookup_key // \"\"" "$CONFIG_FILE")

        if [ -z "$actual_lookup_key_val" ]; then
            print_warning "Lookup key for $env_var_name (mapped from config key: $ref_key) not found or is empty in $CONFIG_FILE (expected at .prices.$ref_key.lookup_key). Skipping."
            echo "# Warning: Lookup key for $env_var_name (mapped from config key: $ref_key) not found in $CONFIG_FILE. Environment variable not set." >> "$ENV_FILE_PATH"
            echo "" >> "$ENV_FILE_PATH" # Add a newline for readability in file
            continue
        fi

        print_status "Fetching Stripe Price ID for lookup_key: '$actual_lookup_key_val' (for $env_var_name)"
        stripe_price_id=$(stripe prices list --lookup-keys "$actual_lookup_key_val" --limit 1 --api-key "$STRIPE_SECRET_KEY" | jq -r '.data[0].id // empty')

        if [ -n "$stripe_price_id" ]; then
            export_command="export $env_var_name=\"$stripe_price_id\""
            echo "$export_command" # Print to stdout
            echo "$export_command" >> "$ENV_FILE_PATH" # Append to file
            print_status "Set: $env_var_name=$stripe_price_id"
        else
            print_warning "Could not find live Stripe Price ID for lookup_key: '$actual_lookup_key_val' (for $env_var_name). Check Stripe dashboard and $CONFIG_FILE."
            echo "# Warning: Could not find live Stripe Price ID for lookup_key: '$actual_lookup_key_val' (for $env_var_name). Environment variable not set." >> "$ENV_FILE_PATH"
        fi
        echo "" >> "$ENV_FILE_PATH" # Add a newline for readability in file
        echo "" # Add a newline for readability in console
    done

    print_status "Finished generating environment variables."
    print_status "Source the file to apply changes: source $ENV_FILE_PATH"
}
